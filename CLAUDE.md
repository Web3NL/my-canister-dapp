...
./scripts/check.sh            # Full prerelease validation (npm ci, build, check, deps:check, rust lint)
```
...
./scripts/setup-dashboard-dev-env.sh  # Configure dashboard for local dev
./scripts/upload-wasm-to-registry.sh  # Upload a wasm to the registry canister
./scripts/copy-example-wasm.sh        # Copy example wasm to wasm/ directory
