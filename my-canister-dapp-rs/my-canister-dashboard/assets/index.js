...
function normalizeZ(c, property, points) {
  ...
  const affined = points.map((p, i3) => p.toAffine(toInv[i3]));
...
function hash_to_field(msg, count, options) {
  ...
  const { p, k: k2, m: m2, hash: hash2, expand, DST: _DST } = options;
...
function weierstrassN(CURVE, curveOpts = {}) {
  ...
  function finishEndo(endoBeta, k1p, k2p, k1neg, k2neg) {
    k2p = new Point(Fp3.mul(k2p.px, endoBeta), k2p.py, k2p.pz);
  ...
  class Point {
    ...
    multiply(scalar) {
      ...
      let point, fake;
      const mul = (n2) => wnaf.wNAFCached(this, n2, Point.normalizeZ);
      if (endo2) {
...
const bls12_381 = bls({
  ...
  G1: {
    ...
    isTorsionFree: (c, point) => {
      ...
      const phi = new c(Fp$1.mul(point.px, beta), point.py, point.pz);
...
function edwards(CURVE, curveOpts = {}) {
  ...
  class Point {
    ...
    multiply(scalar) {
      ...
      const { p, f: f2 } = wnaf.wNAFCached(this, n2, Point.normalizeZ);
...
function eddsa(Point, eddsaOpts) {
  ...
  return { getPublicKey, sign, verify, utils: utils2, Point };
}
