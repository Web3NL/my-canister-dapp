...
import { transferToPrincipal, transferToPrincipalMainnet } from '../helpers';
import { Principal } from '@dfinity/principal';

/**
 * Performs the ICP transfer and cycles top-up flow.
 * Extracted verbatim (logic/comments) from the original test for reuse / clarity.
 */
export async function performTopUp(params: {
    page: Page;
    isMainNet: boolean;
    principal: Principal;
    formattedAmount: string; // Already formatted with formatIcpBalance
}) {
    const { page, isMainNet, principal, formattedAmount } = params;
    ...
    return { cyclesBalanceBefore, cyclesBalanceAfter };
}
